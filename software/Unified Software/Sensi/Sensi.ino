...
void setup() {
  ...
  /******************************************************************************************************/
  // Inject hard coded values into the settungs if needed. Useful when expanding settings and preserving previous EEPROM values.
  ...
  if ( mySettings.debuglevel > 0) { sprintf(tmpStr, "\r\nDebug level is: %d\r\n", mySettings.debuglevel);  printSerialTelnet(tmpStr); }

  /******************************************************************************************************/
  // Start file system and list content
  /******************************************************************************************************/
  if(!LittleFS.begin()){ printSerialTelnet(F("LittleFS mount failed!\r\n")); }
  ...
  if (mySettings.useTelnet         > 0) { mySettings.useTelnet         = true; }
  if (mySettings.useSerial         > 0) { mySettings.useSerial         = true; }
  ...
  // dont search on ports used for data ready or interrupt signaling
        ...
        for (address = 1; address < 127; address++ )  {
          ...
          if (error == 0) {
            ...
            if      (address == 0x20) {
              ...
              sprintf(tmpStr,"Found unknonw device - %d!",address); printSerialTelnet(tmpStr);
            }
          }
        }
      }
  ...
  if (lcd_avail)    { sprintf(tmpStr,"LCD                  available: %d SDA %d SCL %d\r\n", uint32_t(lcd_port),    lcd_i2c[0],    lcd_i2c[1]);    } else { sprintf(tmpStr,"LCD                  not available\r\n"); }  printSerialTelnet(tmpStr);
  if (max_avail)    { sprintf(tmpStr,"MAX30105             available: %d SDA %d SCL %d\r\n", uint32_t(max_port),    max_i2c[0],    max_i2c[1]);    } else { sprintf(tmpStr,"MAX30105             not available\r\n"); }  printSerialTelnet(tmpStr);
  if (ccs811_avail) { sprintf(tmpStr,"CCS811 eCO2, tVOC    available: %d SDA %d SCL %d\r\n", uint32_t(ccs811_port), ccs811_i2c[0], ccs811_i2c[1]); } else { sprintf(tmpStr,"CCS811 eCO2, tVOC    not available\r\n"); }  printSerialTelnet(tmpStr);
  if (sgp30_avail)  { sprintf(tmpStr,"SGP30 eCO2, tVOC     available: %d SDA %d SCL %d\r\n", uint32_t(sgp30_port),  sgp30_i2c[0],  sgp30_i2c[1]);  } else { sprintf(tmpStr,"SGP30 eCO2, tVOC     not available\r\n"); }  printSerialTelnet(tmpStr);
  if (therm_avail)  { sprintf(tmpStr,"MLX temp             available: %d SDA %d SCL %d\r\n", uint32_t(mlx_port),    mlx_i2c[0],    mlx_i2c[1]);    } else { sprintf(tmpStr,"MLX temp             not available\r\n"); }  printSerialTelnet(tmpStr);
  if (scd30_avail)  { sprintf(tmpStr,"SCD30 CO2, rH        available: %d SDA %d SCL %d\r\n", uint32_t(scd30_port),  scd30_i2c[0],  scd30_i2c[1]);  } else { sprintf(tmpStr,"SCD30 CO2, rH        not available\r\n"); }  printSerialTelnet(tmpStr);
  if (sps30_avail)  { sprintf(tmpStr,"SPS30 PM             available: %d SDA %d SCL %d\r\n", uint32_t(sps30_port),  sps30_i2c[0],  sps30_i2c[1]);  } else { sprintf(tmpStr,"SPS30 PM             not available\r\n"); }  printSerialTelnet(tmpStr);
  if (bme280_avail) { sprintf(tmpStr,"BM[E/P]280 T, P[,rH] available: %d SDA %d SCL %d\r\n", uint32_t(bme280_port), bme280_i2c[0], bme280_i2c[1]); } else { sprintf(tmpStr,"BM[E/P]280 T, P[,rH] not available\r\n"); }  printSerialTelnet(tmpStr);
  if (bme680_avail) { sprintf(tmpStr,"BME680 T, rH, P tVOC available: %d SDA %d SCL %d\r\n", uint32_t(bme680_port), bme680_i2c[0], bme680_i2c[1]); } else { sprintf(tmpStr,"BME680 T, rH, P tVOC not available\r\n"); }  printSerialTelnet(tmpStr);
  ...
  if (otaInProgress) { updateOTA(); } // when OTA is in progress we do not do anything else
    ...
    if ((currentTime - lastSYS) >= intervalSYS) {
      ...
      if (mySettings.debuglevel == 99) {
        ...
        sprintf(tmpStr, "Update Time&Date: %4u %4u mDNS:   %4u %4u HTTPupd: %4u %4u HTTP:    %4u %4u\r\nUpdate MQTT:      %4u %4u WS:     %4u %4u NTP:     %4u %4u OTA:     %4u %4u\r\nUpdate Wifi:      %4u %4u \r\n", 
        ...
        printSerialTelnet(tmpStr);
        sprintf(tmpStr, "Update MQTTmsg:   %4u %4u WSmsg:  %4u %4u LCD:     %4u %4u Input:   %4u %4u\r\nUpdate RunTime:   %4u %4u EEPROM: %4u %4u JS:      %4u %4u\r\n", maxUpdateMQTTMESSAGE, AllmaxUpdateMQTTMESSAGE, maxUpdateWSMESSAGE, AllmaxUpdateWSMESSAGE, 
        ...
        printSerialTelnet(tmpStr);
        sprintf(tmpStr, "Update Baseline:  %4u %4u Blink:  %4u %4u Reboot:  %4u %4u AllGood: %4u %4u\r\n", 
        ...
        printSerialTelnet(tmpStr);
        sprintf(tmpStr, "Update this display: %u\r\n", 
                         maxUpdateSYS);
        printSerialTelnet(tmpStr);
        sprintf(tmpStr, "Yield Time:          %u %u %u          ", 
    ...
    if ((currentTime - lastTime) >= intervalRuntime) {
      ...
      lastTime = currentTime;
      if (mySettings.debuglevel >0) { printSerialTelnet(F("Runtime updated\r\n")); }
      #if defined(PROFILE)
      deltaUpdate = millis() - startUpdate;
    ...
    if ((currentTime - lastBlink) > intervalBlink) {                             // blink interval
      ...
      if (blinkLCD) {
        // if (mySettings.debuglevel > 0) { sprintf(tmpStr, "Sensors out of normal range!")); }
        lcd_port->begin(lcd_i2c[0], lcd_i2c[1]);
        lastBlink = currentTime;
...
void timeJSON(char *payLoad) {
      ...
      sprintf(payLoad, "{\"time\":{\"hour\":%d,\"minute\":%d,\"second\":%d,\"microsecond\":%06ld}}",
      localTime->tm_hour,
      localTime->tm_min,
      localTime->tm_sec,
      tv.tv_usec); 
  ...
  if (process) { 
    ...
    if (command == "a") {                                            // set start of night in minutes after midnight
      ...
      if ((tmpuI >= 0) && (tmpuI <= 1440) && (tmpuI <= mySettings.nightBegin)) {
        ...
        sprintf(tmpStr, "Night end set to: %u\r\n", mySettings.nightEnd); printSerialTelnet(tmpStr);
      } else {
        sprintf(tmpStr, "Night end out of valid range 0 - %u\r\n", mySettings.nightBegin); printSerialTelnet(tmpStr);
      }
    }
      ...
      if ((tmpuI >= 0) && (tmpuI <= 99)) {
        ...
        sprintf(tmpStr, "Debug level set to: %u\r\n", mySettings.debuglevel);  printSerialTelnet(tmpStr);
      } else {
    ...
    ///////////////////////////////////////////////////////////////////
      ...
      if ((tmpuI >= 400) && (tmpuI <= 2000)) {
        if (sgp30_avail && mySettings.useSGP30) {
          ...
          sprintf(tmpStr, "Calibration point is: %u\r\n", tmpuI); printSerialTelnet(tmpStr);
        }
      } else {
    ...
    ///////////////////////////////////////////////////////////////////
      ...
      if ((tmpuI >= 0) && (tmpuI <= 100000)) {
        if (ccs811_avail && mySettings.useCCS811) {
          ...
          sprintf(tmpStr, "Calibration point is: %d\r\n",tmpuI); printSerialTelnet(tmpStr);
        }
      } else {
        printSerialTelnet(F("Calibration point out of valid Range\r\n"));
      }
    ...
    ///////////////////////////////////////////////////////////////////
      ...
      #endif  
      EEPROM.put(0, mySettings);
      if (EEPROM.commit()) { sprintf(tmpStr, "EEPROM saved in: %dms\r\n", millis() - tmpTime); } 
      else {sprintf(tmpStr, "EEPROM failed to commit\r\n");} 
      printSerialTelnet(tmpStr);
      ...
      char payLoad[256];
      timeJSON(payLoad);   printSerialTelnet(payLoad); sprintf(tmpStr, " len: %u\r\n", strlen(payLoad)); printSerialTelnet(tmpStr);
      dateJSON(payLoad);   printSerialTelnet(payLoad); sprintf(tmpStr, " len: %u\r\n", strlen(payLoad)); printSerialTelnet(tmpStr);
      bme280JSON(payLoad); printSerialTelnet(payLoad); sprintf(tmpStr, " len: %u\r\n", strlen(payLoad)); printSerialTelnet(tmpStr);
      bme680JSON(payLoad); printSerialTelnet(payLoad); sprintf(tmpStr, " len: %u\r\n", strlen(payLoad)); printSerialTelnet(tmpStr);
      ccs811JSON(payLoad); printSerialTelnet(payLoad); sprintf(tmpStr, " len: %u\r\n", strlen(payLoad)); printSerialTelnet(tmpStr);
      mlxJSON(payLoad);    printSerialTelnet(payLoad); sprintf(tmpStr, " len: %u\r\n", strlen(payLoad)); printSerialTelnet(tmpStr);
      scd30JSON(payLoad);  printSerialTelnet(payLoad); sprintf(tmpStr, " len: %u\r\n", strlen(payLoad)); printSerialTelnet(tmpStr);
      sgp30JSON(payLoad);  printSerialTelnet(payLoad); sprintf(tmpStr, " len: %u\r\n", strlen(payLoad)); printSerialTelnet(tmpStr);
      sps30JSON(payLoad);  printSerialTelnet(payLoad); sprintf(tmpStr, " len: %u\r\n", strlen(payLoad)); printSerialTelnet(tmpStr);
      #if defined(DEBUG)
...
void printSensors() {
  ...
  if (bme280_avail && mySettings.useBME280) {
    char qualityMessage[16];
    sprintf(tmpStr,"BME280 T:%+5.1f[C] P:%5.1f[mbar] P_ave:%5.1f[mbar]", bme280_temp, bme280_pressure/100., bme280_pressure24hrs/100.);  printSerialTelnet(tmpStr);
    if (BMEhum_avail) { sprintf(tmpStr, " rH:%4.1f[%%] aH:%4.1f[g/m^3]\r\n", bme280_hum, bme280_ah); printSerialTelnet(tmpStr); }
  ...
  if (ccs811_avail && mySettings.useCCS811) {
    ...
    sprintf(tmpStr,"CCS811 CO2:%hu tVOC:%hu CCS811_baseline:%hu\r\n", CO2uI, TVOCuI, BaselineuI);  printSerialTelnet(tmpStr);
    checkCO2(float(CO2uI),qualityMessage,15);                                          
    sprintf(tmpStr,"CO2 concentration is %s, ", qualityMessage); printSerialTelnet(tmpStr);
    checkTVOC(TVOCuI,qualityMessage,15);
    sprintf(tmpStr,"tVOC concentration is %s\r\n", qualityMessage); printSerialTelnet(tmpStr);
    sprintf(tmpStr,"CCS811 mode: %d Port: %d SDA %d SCL %d \r\n", ccs811Mode, uint32_t(sgp30_port), sgp30_i2c[0], sgp30_i2c[1]); printSerialTelnet(tmpStr);
  ...
  if (therm_avail && mySettings.useMLX) {
    ...
    sprintf(tmpStr,"MLX Object:%+5.1f[C] MLX Ambient:%+5.1f[C]\r\n", tmpOF, tmpAF); printSerialTelnet(tmpStr);
    float tmpEF = therm.readEmissivity();
    sprintf(tmpStr,"MLX Emissivity:%4.1f MLX Offset:%4.1f\r\n", tmpEF, mlxOffset); printSerialTelnet(tmpStr);
    checkFever(tmpOF,qualityMessage, 15);
    sprintf(tmpStr,"Object temperature is %s, ", qualityMessage);  printSerialTelnet(tmpStr);
    checkAmbientTemperature(tmpOF,qualityMessage,15);
    sprintf(tmpStr,"Ambient temperature is %s\r\n", qualityMessage); printSerialTelnet(tmpStr);
    sprintf(tmpStr,"MLX interval: %d Port: %d SDA %d SCL %d \r\n", intervalMLX, uint32_t(mlx_port), mlx_i2c[0], mlx_i2c[1]); printSerialTelnet(tmpStr);
  } else {
...
void printState() {
  if (wifi_avail && mySettings.useWiFi) {
    // sprintf(tmpStr, "WiFi status: %s ", WiFi.status() ? FPSTR(mON) : FPSTR(mOFF));  printSerialTelnet(tmpStr);
    sprintf(tmpStr, "WiFi host: %s ", hostName); printSerialTelnet(tmpStr);
    if (wifi_connected) {
      sprintf(tmpStr, "is connected to: %s ", WiFi.SSID().c_str()); printSerialTelnet(tmpStr);
      sprintf(tmpStr, "with IP: %s\r\n", WiFi.localIP().toString().c_str()); printSerialTelnet(tmpStr);
    } else {
