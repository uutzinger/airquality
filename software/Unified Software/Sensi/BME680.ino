...
bool initializeBME680() {
  ...
  if (bme680.begin(0x77, true, bme680_port) == true) { 
    ...
    if (mySettings.debuglevel >0) { sprintf(tmpStr, "BME680: interval: %lu\r\n", intervalBME680); printSerialTelnet(tmpStr); }
  ...
  switch(stateBME680) { 
    ...
    case IS_IDLE : { //---------------------
      if ((currentTime - lastBME680) >= intervalBME680) {
        ...
        if (mySettings.debuglevel == 9) { sprintf(tmpStr, "BME680: reading started. Completes in %ldms\r\n", endTimeBME680-tmpTime); printSerialTelnet(tmpStr); }
      }
      break;
    }
    ...
    case HAS_ERROR : {
      ...
      if (mySettings.debuglevel > 0) { printSerialTelnet(F("BME680: has error\r\n")); }
      bme680_avail = false;
      success = false;
      break; 
    }
...
void bme680JSON(char *payload){
  ...
  sprintf(payload, "{\"bme680\":{\"avail\":%s,\"p\":%5.1f,\"pavg\":%5.1f,\"rH\":%4.1f,\"aH\":%4.1f,\"T\":%5.1f,\"resistance\":%d,\"dp_airquality\":\"%s\",\"rH_airquality\":\"%s\",\"resistance_airquality\":\"%s\",\"T_airquality\":\"%s\"}}", 
