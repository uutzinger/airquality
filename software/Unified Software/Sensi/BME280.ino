...
bool initializeBME280() {
  ...
  if (mySettings.debuglevel >0) { sprintf(tmpStr, "BM[E/P]280: interval: %lu\r\n", intervalBME280); printSerialTelnet(tmpStr); }
...
bool updateBME280() {
  ...
  switch(stateBME280) { 
    ...
    case HAS_ERROR : {
      ...
      if (mySettings.debuglevel > 9) { printSerialTelnet(F("BME280: has error!\r\n")); }
      success = false;
      break; 
    }
...
void bme280JSON(char *payload){
  ...
  sprintf(payload, "{\"bme280\":{\"avail\":%s,\"p\":%5.1f,\"pavg\":%5.1f,\"rH\":%4.1f,\"aH\":%4.1f,\"T\":%5.1f,\"dp_airquality\":\"%s\",\"rH_airquality\":\"%s\",\"T_airquality\":\"%s\"}}", 
