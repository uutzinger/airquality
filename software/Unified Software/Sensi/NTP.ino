...
void updateNTP() {
  ...
  switch(stateNTP) {
    ...
    case START_UP : { //---------------------
      ...
      #endif
      if (mySettings.debuglevel == 3) { sprintf(tmpStr, "NTP: connecting to %s\r\n", mySettings.ntpServer); printSerialTelnet(tmpStr); }
...
void processSyncEvent (NTPEvent_t ntpEvent) {
    switch (ntpEvent.event) {
        case timeSyncd:
          ...
          ntp_avail = true;
          timeSynced = true;
          if (mySettings.debuglevel   > 0) { sprintf(tmpStr, "NTP: %s\r\n", NTP.ntpEvent2str(ntpEvent)); printSerialTelnet(tmpStr); }
          break;
        case partlySync:
          // partial sync
          ntp_avail = true;
          timeSynced = true;
          if (mySettings.debuglevel  == 3) { sprintf(tmpStr, "NTP: %s\r\n", NTP.ntpEvent2str(ntpEvent)); printSerialTelnet(tmpStr); }
          break;
        case syncNotNeeded:
          // sync not needed
          ntp_avail = true;
