...
bool initializeCCS811(){
  ...
  if (mySettings.debuglevel >0) { sprintf(tmpStr, "CCS811: Interval: %lu\r\n", intervalCCS811); printSerialTelnet(tmpStr); }
  ...
  if (css811Ret != CCS811Core::CCS811_Stat_SUCCESS) {
    if (mySettings.debuglevel > 0) { sprintf(tmpStr, "CCS811: error opening port - %s\r\n", ccs811.statusString(css811Ret)); printSerialTelnet(tmpStr); }
    ccs811_avail = false;
    success = false;
  }
  
  if (mySettings.debuglevel > 0) { sprintf(tmpStr, "CCS811: begin - %s\r\n", ccs811.statusString(css811Ret)); printSerialTelnet(tmpStr); }
  css811Ret = ccs811.setDriveMode(ccs811Mode);
  if (css811Ret != CCS811Core::CCS811_Stat_SUCCESS) {
    if (mySettings.debuglevel > 0) { sprintf(tmpStr, "CCS811: error setting drive mode - %s\r\n", ccs811.statusString(css811Ret)); printSerialTelnet(tmpStr); }
    ccs811_avail = false;
    success = false;
  }
  
  if (mySettings.debuglevel > 0) { sprintf(tmpStr, "CCS811: mode request set - %s\r\n", ccs811.statusString(css811Ret)); printSerialTelnet(tmpStr); }
  css811Ret = ccs811.enableInterrupts(); // Configure and enable the interrupt line, then print error status
  if (css811Ret != CCS811Core::CCS811_Stat_SUCCESS) {
    if (mySettings.debuglevel > 0) { sprintf(tmpStr, "CCS811: error enable interrupts - %s\r\n", ccs811.statusString(css811Ret)); printSerialTelnet(tmpStr); }
    ccs811_avail = false;
    success = false;
  }
  
  if (mySettings.debuglevel > 0) { sprintf(tmpStr, "CCS811: interrupt configuation - %s\r\n", ccs811.statusString(css811Ret)); printSerialTelnet(tmpStr); }
  if (mySettings.baselineCCS811_valid == 0xF0) {
    ...
    if (css811Ret == CCS811Core::CCS811_Stat_SUCCESS) { 
      ...
      if (mySettings.debuglevel > 0) { sprintf(tmpStr, "CCS811: error writing baseline - %s\r\n", ccs811.statusString(css811Ret)); printSerialTelnet(tmpStr); }
      ccs811_avail = false;
      success = false;
    }
  }
  ...
  switch(stateCCS811) {
    ...
    case DATA_AVAILABLE : { // executed either after sleeping or ideling
      ...
      if ( css811Ret != CCS811Core::CCS811_Stat_SUCCESS) { // Calling this function updates the global tVOC and CO2 variables
        ...
        if (mySettings.debuglevel == 10) { sprintf(tmpStr, "CCS811: readAlgorithmResults completed in %ldms\r\n", (millis()-tmpTime)); printSerialTelnet(tmpStr); }
        ccs811NewData=true;
        ...
        if ((currentTime - lastCCS811Baseline) >= intervalCCS811Baseline ) {
          ...
          if (mySettings.debuglevel == 10) { sprintf(tmpStr, "CCS811: baseline obtained in: %ldms\r\n", (millis()-tmpTime)); printSerialTelnet(tmpStr); }
        }
        ...
        if ((currentTime - lastCCS811Humidity) > intervalCCS811Humidity ) {
          ...
          if (bme680_avail && mySettings.useBME680) {
            ...
            if (mySettings.debuglevel == 10) { sprintf(tmpStr, "CCS811: humidity and temperature compensation updated in %ldms\r\n", (millis()-tmpTime)); printSerialTelnet(tmpStr); }
          }
        }
    ...
    case IS_IDLE : { //---------------------
      ...
      if ( (currentTime - lastCCS811) > 2*intervalCCS811 ) { 
        if (mySettings.debuglevel == 10) { printSerialTelnet(F("CCS811: interrupt timeout occured\r\n")); }
        ccs811_port->begin(ccs811_i2c[0], ccs811_i2c[1]);  
    ...
    case HAS_ERROR : { //-----------------------
      if (mySettings.debuglevel > 0) { printSerialTelnet(F("CCS811: error\r\n")); }
      success = false;
      break;
    }
...
void ccs811JSON(char *payload){
  ...
  sprintf(payload, "{\"ccs811\":{\"avail\":%s,\"eCO2\":%hu,\"tVOC\":%hu,\"eCO2_airquality\":\"%s\",\"tVOC_airquality\":\"%s\"}}", 
