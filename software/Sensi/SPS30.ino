...
bool initializeSPS30() { 
  ...
  if (mySettings.debuglevel > 0) {
    ...
    if (ret == SPS30_ERR_OK) {
      ...
      else { printSerialTelnetLogln(F("not available")); }
  ...
  if (ret != SPS30_ERR_OK) { // I have sensor that reports v.minor = 255 and CRC error when reading version information
    if (mySettings.debuglevel > 0 ) { printSerialTelnetLogln("SPS30: error when reading version information"); } 
    v.major = 1;  // is reasonable minimal version
    v.minor = 0;  // is not reasonable
  }
  if (mySettings.debuglevel > 0 ) {
    snprintf_P(tmpStr, sizeof(tmpStr), PSTR("SPS30: firmware level: %hu.%hu"),v.major,v.minor);             
    printSerialTelnetLogln(tmpStr);
    snprintf_P(tmpStr, sizeof(tmpStr), PSTR("SPS30: hardware level: %hu"),    v.HW_version);                
    printSerialTelnetLogln(tmpStr);
    snprintf_P(tmpStr, sizeof(tmpStr), PSTR("SPS30: SHDLC protocol: %hu.%hu"),v.SHDLC_major,v.SHDLC_minor); 
    printSerialTelnetLogln(tmpStr);
    snprintf_P(tmpStr, sizeof(tmpStr), PSTR("SPS30: library level : %hu.%hu"),v.DRV_major,v.DRV_minor);     
    printSerialTelnetLogln(tmpStr);
  }
  ...
  if (ret == SPS30_ERR_OK) {
    if (mySettings.debuglevel > 0) { 
      snprintf_P(tmpStr, sizeof(tmpStr), PSTR("SPS30: current Auto Clean interval: %us"), autoCleanIntervalSPS30); 
      printSerialTelnetLogln(tmpStr); 
    }
  } else { 
  ...
  switch(stateSPS30) { 
    ...
    case IS_BUSY: { //--------------------- getting system going
      ...
      if ((currentTime - lastSPS30) > 1020) { // start command needs 20ms to complete but it takes 1 sec to produce data
        ...
        if (mySettings.debuglevel >= 2)  { 
          snprintf_P(tmpStr, sizeof(tmpStr), PSTR("SPS30: PM read in %ims"), millis() - tmpTime); 
          R_printSerialTelnetLogln(tmpStr); 
        }
        if (ret == SPS30_ERR_DATALENGTH || valSPS30.MassPM1 == 0) { 
          if (mySettings.debuglevel > 0) { 
            snprintf_P(tmpStr, sizeof(tmpStr), PSTR("SPS30: error data length or zero reading, %u"), sps_error_cnt); 
            R_printSerialTelnetLogln(tmpStr); 
          }
          ...
          if (mySettings.debuglevel > 0) { 
            snprintf_P(tmpStr, sizeof(tmpStr), PSTR("SPS30: error reading values: %hu"), ret); 
            R_printSerialTelnetLogln(tmpStr); 
          }
        ...
        if (mySettings.debuglevel == 5) { 
          snprintf_P(tmpStr, sizeof(tmpStr), PSTR("SPS30: total particles - %f"), totalParticles); 
          R_printSerialTelnetLogln(tmpStr); 
        }
        if (mySettings.debuglevel == 5) {
    ...
    case WAIT_STABLE: { //--------------------- system is stable, read data
      /**if (mySettings.debuglevel == 5) {
        ...
        snprintf_P(tmpStr, sizeof(tmpStr), PSTR("SPS30: current time: %lu"),currentTime); R_printSerialTelnetLogln(tmpStr);
        snprintf_P(tmpStr, sizeof(tmpStr), PSTR("SPS30: time when stable: %lu"), timeSPS30Stable); R_printSerialTelnetLogln(tmpStr);
      *}*/
      if (currentTime >= timeSPS30Stable) {
        ...
        ret = sps30.GetValues(&valSPS30);
        if (mySettings.debuglevel >= 2) { 
          snprintf_P(tmpStr, sizeof(tmpStr), PSTR("SPS30: values read in %ldms"), millis() - tmpTime); 
          R_printSerialTelnetLogln(tmpStr); 
        }
        if (ret == SPS30_ERR_DATALENGTH || valSPS30.MassPM1 == 0.0) { 
          if (mySettings.debuglevel > 0) { 
            snprintf_P(tmpStr, sizeof(tmpStr), PSTR("SPS30: error data length while reading values or zero reading, %u"), sps_error_cnt); 
            R_printSerialTelnetLogln(tmpStr); 
          }
          ...
          if (mySettings.debuglevel > 0) { 
            snprintf_P(tmpStr, sizeof(tmpStr), PSTR("SPS30: error reading values: error x%hu"), ret); 
            R_printSerialTelnetLogln(tmpStr); 
          }
          stateSPS30 = HAS_ERROR;
          errorRecSPS30 = currentTime + 5000;
    ...
    default: {
      if (mySettings.debuglevel > 0) { 
        R_printSerialTelnetLogln(F("SPS30 Error: invalid switch statement"));
      } 
      break;
    }
